  ...
  works: Work[] = [
    {
      ...
      bullets: [
        ...
        '整合OpenLayers 實作地圖即時定位、路線顯示與互動操作。',
        '透過ECharts 視覺化車輛狀態與行為數據，提升資料可讀性。',
    ...
    },
    {
      ...
      bullets: [
        '以 KW Map（勤崴地圖）作為基礎底圖平台，提供管理端更完整的地圖監控與資料分析，協助使用者在電腦環境下進行高效率的車隊管理。',
        '負責前端整體開發與模組拆分，建立可維護的系統架構。',
        '使用Angular 20 + TypeScript 開發主要功能，確保大型系統的穩定性與可擴充性。',
        '整合KW Map建構地圖監控功能。',
        '優化整體介面設計，導入 Isometric 風格科技感插畫。',
        '實作Dark Mode，提升使用體驗。',
        '建立多語系（i18n）架構，以 JSON 管理語系內容，提升系統國際化。',
    ...
    {
      title: '車隊管理系統(系統維護 & 優化)',
      subtitle: '地圖效能與使用體驗優化',
      bullets: [
        '針對既有地圖系統進行效能與使用體驗優化，提升大量地圖資料呈現時的流暢度與穩定性。',
        '軌跡優化：優化歷史軌跡查詢效能，透過延遲渲染依不同 Zoom Level 動態調整軌跡點位數量，解決 2,000 筆以上軌跡資料造成的地圖卡頓問題。',
        '車輛欄位設定優化：優化即時監控欄位設定，實作欄位名稱編輯、順序調整與欄寬設定，透過 localStorage 儲存個人設定，支援不同角色使用者客製化顯示內容。',
        '實作個人化設定匯入 / 匯出功能，將圖層顯示與欄位設定轉為 JSON 檔案，方便跨環境或帳號套用。',
        '圖表優化：使用 ECharts 重構舊有圖表，提升資料視覺化與可讀性。',
      ],
